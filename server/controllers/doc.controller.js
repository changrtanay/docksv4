...
const getAllDocs = async (req, res) => {
  try {
    const publicDocs = await docModel.find({ isPublic: true })
      .select("-doc")
...
const getUserSpecificDocs = async (req, res) => {
  ...
  try {
    const docs = await docModel.find({ author: userId }).populate(
      "author",
      "-password"
    );
...
const getSingleDoc = async (docId) => {
  try {
    const matchedDoc = await docModel.findById(docId);
...
const postDoc = async (req, res) => {
  ...
  try {
    const newDoc = new docModel({ ...req.body, author: userId });
    await newDoc.save();

    res
      .status(201)
      .send({ message: "Document created successfully", data: newDoc });
  } catch (error) {
    console.log("error:", error);
    res.status(500).send({
      message: error.message,
...
const updateDoc = async (req, res) => {
  ...
  try {
    const updatedDoc = await docModel.findByIdAndUpdate(docId, update, {
      new: true,
      runValidators: true,
    });
    /*
    ...
    res
      .status(202)
      .send({
        message: `${updatedDoc.title} is successfully updated`,
        data: updatedDoc,
      });
  } catch (error) {
    console.log("error:", error);
...
const deleteDoc = async (req, res) => {
  ...
  try {
    ...
    res
      .status(202)
      .send({ message: `${deletedDoc.title} is successfully deleted` });
  } catch (error) {
    console.log("error:", error);
    res.status(500).send({
      message: error.message,
