...
import { createDocAction } from "../../redux/doc/doc.actions";
...
function Navbar() {
  ...
  const { loading: createDocLoading } = useSelector(
  ...
  const imageUpload = useCallback(async (e) => {
    ...
    if (imgFile.size > 2097152)
      return toast.error("Please upload image with less-then 2MB size");
  ...
  return (
    <div className={styles.navbar}>
      ...
      <ul>
        <li>
          <button onClick={openModal} className={styles["create-doc"]}>
            Create Doc +
          </button>
        </li>
        <li>
            ...
            <section className={styles["profile-main"]}>
              ...
              {showUserInfo && (
                <div className={styles["profile-info"]}>
                  ...
                  <button onClick={() => navigate("/my-docs")}>
                    My Documents
                  </button>
      ...
      <Modal isOpen={isOpenModal} onClose={closeModal} title="Create Document">
        <form className={styles["create-doc-form"]} onSubmit={handleCreateDoc}>
          ...
          <button type="submit" disabled={createDocLoading}>
            Create Document
          </button>
        </form>
      </Modal>
