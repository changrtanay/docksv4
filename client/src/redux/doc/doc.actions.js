 ...
 * */
export const getAllPublicDocsAction =
  (queryString = "") =>
  async (dispatch) => {
    // start loading
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/docs?${queryString}`,
        {
          headers: {
      ...
      if (res.ok)
        dispatch({
          type: docTypes.DOC_GET_PUBLIC_DOCS_SUCCESS,
          payload: data.data,
        });
...
export const getAllPrivateDocsAction =
  (queryString = "") =>
  async (dispatch) => {
    // start loading
    dispatch({ type: docTypes.DOC_LOADING });
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/docs/user?${queryString}`,
        {
          headers: {
            "Content-Type": "application/json",
            authorization: sessionStorage.getItem("TOKEN"),
      ...
      if (res.ok)
        dispatch({
          type: docTypes.DOC_GET_PRIVATE_DOCS_SUCCESS,
          payload: data.data,
        });
      else dispatch({ type: docTypes.DOC_ERROR });
    } catch (error) {
...
export const createDocAction =
  ...
  async (dispatch) => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/docs`, {
        method: "POST",
 ...
 * */
export const updateDocAction =
  ({ docId, update, updateDocState }) =>
  async (dispatch) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/docs/${docId}`,
        {
          method: "PATCH",
      ...
      if (res.ok) {
        dispatch({ type: docTypes.DOC_SUCCESS });
        updateDocState(
          update.isPublic === undefined ? data.data.title : data.data.isPublic
        );
      } else dispatch({ type: docTypes.DOC_ERROR });
